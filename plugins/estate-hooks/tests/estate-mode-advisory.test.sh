...
ENR="$(mktemp -d)"
mkdir -p "$ENR/.config/claude-estate"
: >"$ENR/.config/claude-estate/estate-mode.gitconfig"
# not-enrolled scratch HOME (dir exists, gitconfig absent)
UNE="$(mktemp -d)"
mkdir -p "$UNE/.config/claude-estate"
...
rc=0
[[ "$out" == *"Employer identity mode"* ]] || rc=1
