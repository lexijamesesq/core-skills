...
- **C6 — WIP override.** Before `--caller-ack-wip`: a related/dependent chain, or a silent switch? Caller — its own intent; disposition rule is `/linear`'s `playbooks/claim.md` Step 2 law, absorbed here by pointer. On a shared app actor, C6 is cross-session — every peer writes to Linear as the same actor, so C5 and C6 fire across sessions, not just within one. Identify the peer holding the other In Progress ticket, then pass `--caller-ack-wip`; a Planning ticket delegated to the actor is yours to begin if you or your predecessor window claimed it — that's `begin`, not a second `claim`. Never pass `--operator-directed` on a peer's relay — that flag asserts the operator herself directed this claim, not that a peer session did.
