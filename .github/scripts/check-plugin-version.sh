...
# .pre-commit-config.yaml is repo-level infrastructure (a dotty pin bump,
# say), never plugin content -- excluded from the diff regardless of
# PLUGIN_DIR. For a single-plugin repo (PLUGIN_DIR=".", the whole repo is
# the plugin) this file sits inside PLUGIN_DIR and, unexcluded, made every
# dotty-bump PR fail this check for a change that isn't plugin content at
# all (found live: wiki and publish-skills both needed a hand version
# bump on every dotty-bump PR for this exact reason). A no-op exclude for
# a multi-plugin repo (PLUGIN_DIR a subdirectory) where the file was never
# inside PLUGIN_DIR to begin with.
if git diff --quiet "$LATEST_TAG" HEAD -- "$PLUGIN_DIR" ":(exclude)${PLUGIN_DIR%/}/.pre-commit-config.yaml"; then
